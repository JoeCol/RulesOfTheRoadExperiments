...
generateActions(r225a, B, I, R) :- member(maxPossibleSpeed25orLess, B), ember(onUnrestictedDualCarriageway, B), generateActions(r226, B, I, Y), append([must-flash_amber_beacon],Y,R), !.
...
generateActions(r249, B, I, R) :- member(parked, B), member(limitabove30mph), generateActions(r250, B, I, Y), append([must-parking_lights_on],Y,R), !.
generateActions(r249, B, I, R) :- generateActions(r250, B, I, R), !.

%Rule Description: ParkingLights regular vehicles
generateActions(r250, B, I, R) :- member(parked, B), \+member(limitabove30mph), \+member(parkingSpace), \+member(toJunction10meters, B), generateActions(r251, B, I, Y), append([must-parking_lights_on],Y,R), !.
...
generateActions(r260, B, I, R) :- member(motorway, B), \+member(visibilityReduced), generateActions(r261, B, I, Y), append([should-steady_speed, should-safe_distance],Y,R), !.
...
generateActions(r263, B, I, R) :- member(motorway, B), \+member(directedByPoliceOfficer), generateActions(r264, B, I, Y), append([must-avoid_drive_against_traffic_flow, must-avoid_crossing_central_reservation, must-avoid_reversing],Y,R), !.
generateActions(r263, B, I, R) :- generateActions(r264, B, I, R), !.

%Rule Description: Stay in leftMostLane
generateActions(r264, B, I, R) :- member(motorway, B), \+member(overtaking, I), \+member(hardshoulderUsed), generateActions(r266, B, I, Y), append([should-keep_left_lane],Y,R), !.
...
generateActions(r296, B, I, R) :- member(stopSignCrossing, B), (\+member(greenLight, B); member(redLight)), generateActions(r297, B, I, Y), append([must-avoid_crossing_crossing],Y,R), !.
...
generateActions(r302, B, I, R) :- member(stop, I), member(tramlines), generateActions(r303, B, I, Y), append([must-avoid_parking],Y,R), !.
