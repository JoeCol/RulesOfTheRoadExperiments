...
public class RudeCar extends AbstractROTRCar implements CarEvents
{
	...
	protected ArrayDeque<Direction> getSimulationRoute()
	{
		...
		else if (intentions.get(CarIntention.CI_turnRight) && atRightTurn)
		{
			setSpeed(currentLimit);
			movement.add(Direction.east);
	...
	public void worldUpdate(WorldSim world, Point location) 
	{
		...
		currentLimit = world.speedLimit(location.getX(), location.getY());
